...
router.post('/', async (req: Request, res: Response): Promise<void> => {
    try {
        const { reference, suffix, receiptText, fullText } = req.body;
        const suppliedReceiptText = typeof receiptText === 'string' ? receiptText : fullText;
        
        // Validate required parameters
        if (!reference || (!suffix && !suppliedReceiptText?.trim())) {
            logger.warn('❌ Missing required parameters for Abyssinia verification');
            res.status(400).json({
                success: false,
                error: 'Missing required parameters: reference and suffix are required unless receiptText is provided'
            });
            return;
        }
        
        // Validate parameter types
        if (typeof reference !== 'string' || (suffix !== undefined && typeof suffix !== 'string')) {
        ...
        if (suffix && (suffix.length !== 5 || !/^\d{5}$/.test(suffix))) {
        ...
        const result = suppliedReceiptText?.trim()
            ? verifyProviderReceiptText('ABYSSINIA', reference, suppliedReceiptText)
            : await verifyAbyssinia(reference, suffix);
