...
import { verifyCBE, verifyCBEFromText } from '../services/verifyCBE';
...
router.post('/', async function (
    ...
    const { reference, accountSuffix, receiptText, fullText } = req.body;
    ...
    if (isLegacyReference && !trimmedSuffix && !suppliedReceiptText?.trim()) {
        res.status(400).json({ success: false, error: 'Legacy CBE verification requires accountSuffix or receiptText.' });
        return;
    }

    try {
        const result = suppliedReceiptText?.trim()
            ? verifyCBEFromText(normalizedReference, suppliedReceiptText)
            : await verifyCBE(normalizedReference, trimmedSuffix);
        if (!result.success) {
...
router.get('/', async function(
    req: Request<{}, {}, {}, { reference?: string; accountSuffix?: string; receiptText?: string; fullText?: string }>,
    res: Response
): Promise<void> {
    const { reference, accountSuffix, receiptText, fullText } = req.query;
    const suppliedReceiptText = typeof receiptText === 'string' ? receiptText : fullText;
